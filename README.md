...
## 检查一个Xcode项目
* FauxPas执行检查在**Xcode项目**中的**单个target**上，使用单个的**项目配置**.在最简化的情况下，项目必须被指定————FauxPas会选择一个默认的target和build配置，如果它们没有被指定

### 选择哪些规则去执行
* 规则可以通过它们的标签被指定，或单独地指定。单独的规则也可以被排除。排除的动作将被最后执行，所以如果你排除了一个规则，但又选择了一个包含这条规则的标签，这条规则将不会被执行
